...
blocks{1}.sequence=1;
blocks{1}.msec=16000;
blocks{1}.slicing=1000/8;
blocks{1}.repetitions=1;
blocks{1}.name='Fixation';

for ii=1:1:6
  blocks{2+2*(ii-1)}.randomization=0;
  blocks{2+2*(ii-1)}.sequence=repmat(repmat(1:8,[1,2]),[1,16])+1; % (16 s + 16 s) x 6 repetitions
  blocks{2+2*(ii-1)}.msec=1000/8/2*ones(1,8*2*16);
  blocks{2+2*(ii-1)}.slicing=1000/8;
  blocks{2+2*(ii-1)}.repetitions=1;
  blocks{2+2*(ii-1)}.name='16 s checkerboard';

  blocks{3+2*(ii-1)}.randomization=0;
  blocks{3+2*(ii-1)}.sequence=1;
  blocks{3+2*(ii-1)}.msec=16000;
  blocks{3+2*(ii-1)}.slicing=1000/8;
  blocks{3+2*(ii-1)}.repetitions=1;
  blocks{3+2*(ii-1)}.name='Fixation';
end

blocks{3+2*(ii-1)+1}.randomization=0;
blocks{3+2*(ii-1)+1}.sequence=1;
blocks{3+2*(ii-1)+1}.msec=16000;
blocks{3+2*(ii-1)+1}.slicing=1000/8;
