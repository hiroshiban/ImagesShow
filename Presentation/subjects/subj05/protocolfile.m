...
for ii=1:1:10
  blocks{2+2*(ii-1)}.randomization=0;
  blocks{2+2*(ii-1)}.sequence=[1:2:50;2:2:50]+1;
  blocks{2+2*(ii-1)}.msec=100*ones(1,size(blocks{2+2*(ii-1)}.sequence,2));
  blocks{2+2*(ii-1)}.slicing=100;
  blocks{2+2*(ii-1)}.repetitions=1;
  blocks{2+2*(ii-1)}.name=sprintf('N2F %02d',2+2*(ii-1));

  blocks{3+2*(ii-1)}.randomization=0;
  blocks{3+2*(ii-1)}.sequence=flipdim([1:2:47;2:2:48]+1,2);
  blocks{3+2*(ii-1)}.msec=100*ones(1,size(blocks{3+2*(ii-1)}.sequence,2));
  blocks{3+2*(ii-1)}.slicing=100;
  blocks{3+2*(ii-1)}.repetitions=1;
  blocks{3+2*(ii-1)}.name=sprintf('F2N %02d',2+2*(ii-1));
end

blocks{3+2*(ii-1)+1}.randomization=0;
blocks{3+2*(ii-1)+1}.sequence=1;
blocks{3+2*(ii-1)+1}.msec=5000;
blocks{3+2*(ii-1)+1}.slicing=100;
blocks{3+2*(ii-1)+1}.repetitions=1;
blocks{3+2*(ii-1)+1}.name='Fixation';
