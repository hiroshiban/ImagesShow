...
%             By properly setting the 'position' (in the option file) and related parameters (here in the
%             image_database file), ImagesShow offers two different ways to record stimulus/condition types
%             and presentation protocols.
%             First, if you set 'position' to one of 1-12, while the stimulus onset marker (punch stimulus)
%             is only presented at a fixed location, you can record stimulus types in an eventlogger object
...
%             imgdb.img{1}{3}=[1], imgdb.img{2}{3}=[1,2], and imgdb.img{3}{3}=[2,3], then an onset marker
%             will be presented on top-left for the image 1, two markers on top-left and top-right for the
%             image 2, and two markers on top-right and bottom-left for the image 3. Then, when you record
%             the photo-trigger responses on an EEG/MEG system, etc, you can judge which stimuli are
%             presented in which order by checking the recorded binary codes, e.g. '11000000'. For such
%             multiple recordings, the combinations of numbers 1-8 are recommended, since the bar-type
%             triggers (position 9-12) are not suitable for binary coding.
...
% Last Update: "2025-02-18 22:22:24 ban"
